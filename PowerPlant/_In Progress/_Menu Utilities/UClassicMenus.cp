...
// ===========================================================================
//	UClassicMenus.cp			PowerPlant 2.2.2	©1998-2005 Metrowerks Inc.
// ===========================================================================
...
#include <UClassicMenus.h>
		...
		// Carbon does not support Classic menus. You must use
		// UAppearanceMenus instead. Remove this file from Carbon targets

	#error "Carbon does not support Classic menus"
...
void
UClassicMenus::SetItemHierID(
	MenuHandle		inMacMenuH,
	UInt16			inMenuItem,
	ResIDT			inMenuID )
{
	::SetItemCmd(inMacMenuH, (SInt16) inMenuItem, hMenuCmd);
	::SetItemMark(inMacMenuH, (SInt16) inMenuItem, inMenuID);
}
...
SInt16
UClassicMenus::GetItemHierID(
	MenuHandle		inMacMenuH,
	UInt16			inMenuItem )
{
	SInt16	theID = 0;

	// Make sure it has a hierarchical menu in the first place
	SInt16 theCmd;
	::GetItemCmd( inMacMenuH, (SInt16) inMenuItem, &theCmd );
	if ( theCmd == hMenuCmd ) {
		// It does. Get the ID.
		::GetItemMark( inMacMenuH, (SInt16) inMenuItem, &theID );
	}
...
void
UClassicMenus::SetItemTextEncoding(
	MenuHandle		inMacMenuH,
	UInt16			inMenuItem,
	TextEncoding	inEncoding )
{
	::SetItemCmd( inMacMenuH, (SInt16) inMenuItem, 0x1C );
	::SetItemIcon( inMacMenuH, (SInt16) inMenuItem, static_cast<SInt16>(inEncoding) );
}
...
TextEncoding
UClassicMenus::GetItemTextEncoding(
	MenuHandle		inMacMenuH,
	UInt16			inMenuItem )
{
	TextEncoding	theEncoding = 0;

	// make certain we have encoding, so we don't accidentally
	// return an icon ResIDT

	SInt16 theCmd;
	::GetItemCmd( inMacMenuH, (SInt16) inMenuItem, &theCmd );
	if ( theCmd == 0x1C ) {
		SInt16 theIcon;
		::GetItemIcon( inMacMenuH, (SInt16) inMenuItem, &theIcon );
		theEncoding = theIcon;
	}
...
void
UClassicMenus::SetItemKeyEquivalent(
	MenuHandle		inMacMenuH,
	UInt16			inMenuItem,
	SInt16			inKey,
	UInt8			/* inModifiers */,
...
void
UClassicMenus::GetItemKeyEquivalent(
	MenuHandle		inMacMenuH,
	UInt16			inMenuItem,
	SInt16			&outKey,
	UInt8			&outModifiers,
	SInt16			&outGlyph )
{
	...
	if ( outKey != 0 ) {
		// no need to set outModifers.. it could only be the cmdKey
		// in this case. And the glyph will be the same as the key.
